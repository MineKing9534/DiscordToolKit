...
interface IComponent<C : Component> {
    ...
    fun transform(mapper: (() -> List<C>) -> List<C>): IComponent<C>
}

@Suppress("UNCHECKED_CAST")
fun <C : Component, W : IComponent<C>> W.show(show: Boolean = true) = transform { if (show) it() else emptyList() } as W
fun <C : Component, W : IComponent<C>> W.hide(hide: Boolean = true) = show(!hide)

@Suppress("UNCHECKED_CAST")
fun <C : IDisableable, W : IComponent<C>> W.enabled(enabled: Boolean = true) = transform { it().map { it.withDisabled(!enabled) as C } } as W
