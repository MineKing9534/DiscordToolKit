...
class UIManager(manager: DiscordToolKit<*>) : Manager(manager) {
    ...
    fun <M> registerMenu(name: String, defer: DeferMode = DEFAULT_DEFER_MODE, localization: LocalizationFile? = null, init: MessageMenuConfigurator<M>): MessageMenu<M, *> {
        val builder = MessageMenuConfigImpl(MenuConfigPhase.BUILD, null, MenuInfo.create(name, this), localization) { init() }
        builder.init()
        return registerMenu(name, defer, localization, builder, init)
    }

    fun <M> registerMenu(name: String, defer: DeferMode = DEFAULT_DEFER_MODE, localization: LocalizationFile? = null, config: MenuConfigData, init: MessageMenuConfigurator<M>): MessageMenu<M, *> {
        return registerLocalizedMenu<M, LocalizationFile?>(name, defer, localization, config) { init() }
    }

    inline fun <M, reified L : LocalizationFile> registerLocalizedMenu(name: String, defer: DeferMode = DEFAULT_DEFER_MODE, noinline init: LocalizedMessageMenuConfigurator<M, L>): MessageMenu<M, L> {
        val file = manager.localizationManager.read<L>()
        return registerLocalizedMenu(name, defer, file, init)
    }

    fun <M, L : LocalizationFile?> registerLocalizedMenu(name: String, defer: DeferMode = DEFAULT_DEFER_MODE, localization: L, init: LocalizedMessageMenuConfigurator<M, L>): MessageMenu<M, L> {
        ...
        return registerLocalizedMenu(name, defer, localization, builder, init)
    }

    inline fun <M, reified L : LocalizationFile> registerLocalizedMenu(name: String, defer: DeferMode = DEFAULT_DEFER_MODE, config: MenuConfigData, noinline init: LocalizedMessageMenuConfigurator<M, L>): MessageMenu<M, L> {
        val file = manager.localizationManager.read<L>()
        return registerLocalizedMenu(name, defer, file, config, init)
    }

    fun <M, L : LocalizationFile?> registerLocalizedMenu(name: String, defer: DeferMode = DEFAULT_DEFER_MODE, localization: L, config: MenuConfigData, init: LocalizedMessageMenuConfigurator<M, L>): MessageMenu<M, L> {
        require(!menus.containsKey(name)) { "Menu $name already defined" }

        val menu = MessageMenu(this, name, defer, localization, config.setup, config.states, init)
        menus[name] = menu
